...
def srcsetThumb(data, forTag=False):
    thumb = BeautifulSoup("lxml").new_tag("img")
    thumb['src'] = "/"+str(data)
    if forTag:
        soup = srcsets(thumb, False, forTag=True)
    else: 
...
def findLink(text, forTag=False):
    if forTag:
        return re.search(r"/tags/(?P<tag>\S*)/(?P<file>\S*)\.(?P<ext>\w*)", str(text))
    else:
        return re.search(r"/(?P<year>\d{4})/(?P<month>\d{1,2})/(?P<day>\d{1,2})/(?P<file>\S*)\.(?P<ext>\w*)", str(text))

def findFile(link, forTag=False):
    if link:
        if forTag:
            return '/root/myblog/myblog/blog/static/media/tags/{}/{}.{}'\
            .format(link.group("tag"),link.group("file"),link.group("ext"))
        else:
...
def saveImage(link, file, sz, forTag=False ):
    if forTag:
        
        file_out = '/root/myblog/myblog/blog/static/media/tags/{}/{}-{}.{}'\
        .format(link.group("tag"),uri_to_iri(link.group("file")),\
        sz, link.group("ext"))
        link_out = '/media/tags/{}/{}-{}.{}'\
        .format(link.group("tag"),link.group("file"),\
        sz, link.group("ext"))
...
def srcsets(text, wrap_a, forTag=False):
    """Make few srcsets"""
    ...
    if len(img_links) != 0:
        for i in img_links: # для каждой
            ...
            if forTag:
                link = findLink(iri_to_uri(i),forTag=True)
                file = uri_to_iri(findFile(link,forTag=True))
                original_pic = '/media/tags/{}/{}.{}'.\
                    format(link.group("tag"),uri_to_iri(link.group("file")),\
                       link.group("ext"))
            else:
            ...
            if os.path.isfile(file):
                ...
                if notgif:
                    for sz in reversed(src_szs):
                        if w > sz:
                            srcset[sz] = saveImage(link, file, sz, forTag=True )
                    print("**************************")
                    ...
                    if 1600 in srcset:
                        ...
                        srcset[1920] = saveImage(link, file, 1920 ,forTag=True )
                    elif 1366 in srcset:
                        srcset[1600] = saveImage(link, file, 1600 ,forTag=True)
                        alt = srcset[1366]
                    elif 800 in srcset:
                        srcset[1366] = saveImage(link, file, 1366 ,forTag=True)
                        alt = srcset[1366]

                    elif 480 in srcset:
                        alt = original_pic
                        srcset[800] = saveImage(link, file, 800 ,forTag=True)
                    else:
                        alt = original_pic
